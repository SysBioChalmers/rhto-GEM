...
% newRelease
%   Prepares a new release of the Rhto model, for direct submission to
...
%Bump version number:
oldModel   = load('../ModelFiles/mat/rhto.mat');
...
fclose(fid);
if ~contains(history,['rhtov' newVersion ':'])
...
model.description = ['Rhodosporidium toruloides-GEM_v' newVersion];

%Save model
exportForGit(model,'rhto','..',{'mat', 'txt', 'xlsx', 'xml', 'yml'});
